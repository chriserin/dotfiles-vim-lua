...
function M.get_tests()
  ...
  for _, captures, _ in query:iter_matches(root, bnr) do
    ...
    for _, existing_block in ipairs(result) do
      if
        existing_block.describe_start < test_table.describe_start
        and existing_block.describe_end > test_table.describe_end
      then
...
function M.run_jest_test()
  ...
  if not (parser_language == 'typescript' or parser_language == 'tsx' or parser_language == 'javascript') then
    print('Not a jest language: ' .. parser_language)
    return
  end
